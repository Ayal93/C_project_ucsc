...
void run_auction(Square *square, Player players[], int num_players)
{
    ...
    //  Bidding starts at 50% of  value
    double current_bid = square->property.Purchase_Price * 0.50;
    printf("Opening Bid :\nLKR %.0f.\n\n", current_bid);
 
    // Track active biddersa (1 = active, 0 = withdrawn , bankrupt)
...
else
{
    next_min_bid = current_bid + 250.0;
        ...
        //  Cannot bid more cash than currently available
...
// auctions all properties of bankrupt players
 
void auction_liquidated_assets(Player *bankrupt_player, Board *board, Player players[], int num_players)
{
 
    for (int i = 0; i < bankrupt_player->number_of_properties; i++)
    {
        // Find matching property square on the main board
            for (int k = 0; k < 40; k++)
        {
            if (board->squares[k].square_type == Property &&
                board->squares[k].property.Current_Owner == bankrupt_player->player_order)
              {
                // Reset ownership back to Bank
