...
void check_bankrupt(Player *player){
    if (player->is_bankrupt)
    {       printf("Player %s is bankrupt out of game.",&player->player_name);
            return;
    }
     if (player->player_cash_in_hand < 0)
    {
        attempt_debt_recovery(player);
    }
...
void pay_rent_when_land_on_others(Player *player, Square *Square_current, int player_index)
{
        ...
        Square_current->property.Current_Owner != player_index)
    {
        ...
        printf("\n%s landed on %s\n", player->player_name, Square_current->square_name);
        printf("Rent paid: LKR %.2f\n", Square_current->property.Base_Rental);
        printf("Owner: %s", players[owner_index].player_name);

        player->player_cash_in_hand -= Square_current->property.Base_Rental;
        players[owner_index].player_cash_in_hand += Square_current->property.Base_Rental;
        players[owner_index].rental_income += Square_current->property.Base_Rental;
    }
...
void player_build_dission(Player *player, Square *square, int player_index) {

    switch (player->name_by_enum) {

        case Aggressive_Investor:
            while (square->property.no_of_House_Construction < 4 &&
                ...
                printf("%s constructed one house on %s. Total houses: %d\n",
                       player->player_name, square->square_name,
                       square->property.no_of_House_Construction);
            }
        ...
        case Conservative_Banker:
            if (player->number_of_loans == 0 &&
                square->property.no_of_House_Construction < 4 &&
                square->property.no_of_Hotel_Construction == 0 &&
                square->property.no_of_House_Construction <= min_houses_in_group(square->property.group) &&
                (player->player_cash_in_hand - square->property.House_Construction_Cost)
...
void print_game_over(Player *winner)
{
    ...
    double net_worth = winner->player_cash_in_hand + total_property_value - winner->loan_amount;
